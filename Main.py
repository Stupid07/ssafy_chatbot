...
# 처음 실행할 때 자신의 slack정보 입력
slack_token = None
slack_client_id = None
slack_client_secret = None
slack_verification = None
# 처음 실행할때 위의 정보를 입력후 이 값을 True 로 그 후엔 False
bSaveSlackData = False


#파일로 저장하는 함수
def init_slack_data():
    with open("slackData.txt","wb") as file:
        pickle.dump(slack_token, file)
        pickle.dump(slack_client_id, file)
        pickle.dump(slack_client_secret, file)
        pickle.dump(slack_verification, file)
...
# 이후에는 파일을 통해 슬랙 정보 불러오기
with open("slackData.txt", "rb") as file:
    slack_token = pickle.load(file)
    slack_client_id = pickle.load(file)
    slack_client_secret = pickle.load(file)
    slack_verification = pickle.load(file)
